...
class PANXDataset():
    def __init__(self, tokenizer: AutoTokenizer, data_args, training_args) -> None:
        ...
        raw_datasets = load_dataset(f'tasks/panx/datasets/{data_args.dataset_name}.py', name=data_args.lang)
    ...
    def tokenize_and_align_labels(self, examples):
        ...
        for i, label in enumerate(examples[self.label_column_name]):
            ...
            # word_ids = word_ids if len(word_ids)<512 else word_ids[:512]
            word_ids += [None]
            ...
            #     label_ids = label[:511]
            #     label_ids.append(-100)
            # assert len(label_ids)<512, f"length of label ids is larger than 512: {len(label_ids)}"
