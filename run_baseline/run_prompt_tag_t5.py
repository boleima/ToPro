...
from preprocessor import T5Preprocessor
...
def train(args, train_dataset, model, tokenizer, preprocessor, label_list, lang2id=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": torch.squeeze(batch[0],dim=1), "attention_mask": torch.squeeze(batch[1],dim=1), "labels": torch.squeeze(batch[2],dim=1)}
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for idx in range(args.num_priming):
            ...
            for batch in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                inputs = {"input_ids": torch.squeeze(batch[0],dim=1), "attention_mask": torch.squeeze(batch[1],dim=1), "labels": torch.squeeze(batch[2],dim=1)}
                with torch.no_grad():
                    generated_ids = model.generate(input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"], num_beams=args.num_beam, min_length = 1, max_length=5, early_stopping=True)
...
def load_and_cache_examples(args, task, preprocessor, split='train', language='en', lang2id=None, evaluate=False,
    ...
    all_label_ids = torch.stack([f.label for f in features])
...
def main():
    ...
    preprocessor = T5Preprocessor(tokenizer, label_list, args.max_seq_length, args.task_name, args.pattern_id)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if args.fp16:
            model = model.half()
        model.to(args.device)
        output_predict_file = os.path.join(args.output_dir, args.test_split + str(args.seed)+'_results.txt')
        total = total_correct = 0.0
        with open(output_predict_file, 'a') as writer:
