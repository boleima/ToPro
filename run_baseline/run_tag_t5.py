...
MODEL_CLASSES = {
  ...
  't5': (MT5Config, MT5ForConditionalGeneration, MT5Tokenizer)
}
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if 't5' not in args.model_name_or_path:
        ...
        inputs = {"input_ids": torch.squeeze(batch[0],dim=1),
            "attention_mask": torch.squeeze(batch[1],dim=1),
            "labels": torch.squeeze(batch[2],dim=1)}
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
      ...
      with torch.no_grad():
        inputs = {"input_ids": torch.squeeze(batch[0],dim=1),
            "attention_mask": torch.squeeze(batch[1],dim=1),
            "labels": torch.squeeze(batch[2],dim=1)}
        generated_ids = model.generate(input_ids = inputs["input_ids"],attention_mask=inputs["attention_mask"], num_beams=args.num_beam, min_length = 10, max_length=args.max_target_length, early_stopping=True)
        generated_text = tokenizer.batch_decode(generated_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
        target_text = tokenizer.batch_decode(inputs['labels'], skip_special_tokens=True, clean_up_tokenization_spaces=False)

        target_texts.extend(target_text)
        generated_texts.extend(generated_text)
    results = span_f1(target_texts, generated_texts)
    preds_list = generated_texts
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    for lg in langs:
      ...
      if 't5' not in args.model_name_or_path:
        ...
        features_lg = convert_examples_to_features_t5(examples, args.max_seq_length, args.max_target_length, tokenizer,
                          task=args.task,
                          lang=lg)
  ...
  if  args.few_shot_percentage > 0 and mode == 'train':
    logger.info("Original no. of examples = {}".format(len(features)))
  ...
  all_label_ids = torch.stack([f.label_ids for f in features])
  if args.model_type == 'xlm' and features[0].langs is not None:
    ...
    if 't5' in args.model_name_or_path:
      #all_target_text = [f.target for f in features]
...
def main():
  ...
  parser.add_argument("--task", default="panx", type=str,
            help="Different task name used for adding prefix for the input")
  ...
  parser.add_argument("--max_target_length", default=150, type=int,
            help="The maximum total output sequence length after tokenization. Sequences longer "
               "than this will be truncated, sequences shorter will be padded.")
  parser.add_argument("--num_beam", default=3, type=int,
            help="The number of beam search for text generation.")
  ...
  parser.add_argument("--do_prompt_tuning", action="store_true",
            help="Whether to run training.")
  ...
  if args.do_prompt_tuning:
    ...
    prompt_tuning_init_text="Generate the part-of-speech tags of the sentence: " if args.task=='udpos' else 'Generate the named entity recognition tags of the sentence: ',
    tokenizer_name_or_path=args.tokenizer_name if args.tokenizer_name else args.model_name_or_path)
    model = get_peft_model(model, peft_config)
    logger.info("Prompt tuning trainable parameters:")
  ...
  if args.do_train:
    ...
    output_test_results_file = os.path.join(args.output_dir, str(args.seed)+"_test_results.txt")
  ...
  if args.do_predict_dev and args.local_rank in [-1, 0]:
    ...
    output_test_results_file = os.path.join(args.output_dir, str(args.seed)+"dev_results.txt")
