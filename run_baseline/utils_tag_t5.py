...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, label_ids, segment_ids=None,langs=None, target=None):
...
def convert_examples_to_features_t5(examples,
                 max_seq_length,
                 max_target_length,
                 tokenizer,
                 task,
  ...
  prefix = 'tag: ' if task == 'panx' else 'POS tagging: '
  features = []
  for (ex_index, example) in enumerate(examples):
    if ex_index % 10000 == 0:
      logger.info("Writing example %d of %d", ex_index, len(examples))

    sent = prefix + ' '.join(example.words)
    target = ''
    for word, label in zip(example.words, example.labels):
      target += label + ': '+word + ' $$ '
    target = target[:-4]

    model_input = tokenizer(text = sent, max_length=max_seq_length,padding='max_length',truncation = True, return_tensors="pt" )
    model_target = tokenizer(text = target, max_length=max_target_length,padding='max_length',truncation = True, return_tensors="pt" )
    ...
    features.append(
        InputFeatures(input_ids=model_input.input_ids,
                input_mask=model_input.attention_mask,
                label_ids=model_target.input_ids,
                target=target,
                langs=langs))
  return features
